...
class Ator():
    ...
    def colidir(self, outro_ator, intervalo=1):
        ...
        if self.status == ATIVO and outro_ator.status == ATIVO:
            delta_x = abs(self.x - outro_ator.status.x)
            delta_y = abs(self.y - outro_ator.status.y)
        elif delta_x <= intervalo and delta_y <= intervalo:
...
class Passaro(Ator):
    ...
    def calcular_posicao(self, tempo):
        ...
        if self._esta_voando():
            delta_t = tempo-self_tempo_de_lancamento
